...
function deleta(codigo) {
  ...
  acao.value = 'd';
  id.value = codigo;
  const formulario = document.getElementById('frmaluno');
  // Instância o FormData passando como parâmetro o formulário
  const formData = new FormData(formulario);
  $.ajax({
    type: "post",
    url: "controlealuno.php",
    data: formData,
    //dataType: 'json',
    processData: false,
    contentType: false,
        ...
        success: function (data) {
          if (data != 'Falha no cadastro') {
            ...
            if (acao.value == 'c') {
              ...
              setTimeout(function () {
                window.location.replace('listaaluno');
              }, 1000);
            } else {
